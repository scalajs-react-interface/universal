...
object UniversalRouter {
  ...
  object MobileRouterContext {
    ...
    def apply(
        ctrl: UniversalRouterCtrl,
        key: js.UndefOr[String] = js.undefined,
        ref: js.Function1[Component, Unit] = null)(children: ReactNode*) =
      makeElementWithChildren[Component](Props(ctrl), key = key, ref = ref)(
        children: _*)
  ...
  class Component extends ReactComponent[Props, Unit] {
    ...
    override def render(): ReactElement = {
      Navigator(new NavigatorProps {
        override var renderScene: js.Function2[js.Dynamic,
                                               NavigatorM,
                                               ReactElement] = renderSceneF _
        ref = js.defined(storeNavRef _)
        style = props.style
        sceneStyle = props.sceneStyle
        initialRoute = props.config.initialRoute._2
        configureScene = js.defined(configureSceneF _)
      })
    }
    ...
    def storeNavRef(navigator: NavigatorM): Unit = {
      if (navigator == null) {
        // when unmounting
    ...
    def subscribeFocusEvents(navigator: NavigatorM) = {
      ...
      willFocusSubscription =
        navContext.addListener("willfocus", (event: js.Dynamic) => {
          if (props.config.onWillFocus != null)
            props.config.onWillFocus(
              event.data.route.asInstanceOf[NavigatorRoute])
        })
      didFocusSubscription =
        navContext.addListener("didfocus", (event: js.Dynamic) => {
          if (props.config.onDidFocus != null)
            props.config.onDidFocus(
              event.data.route.asInstanceOf[NavigatorRoute])
        })
    }
    ...
    def configureSceneF(route: js.Dynamic) = {
      if (!js.isUndefined(route.sceneConfig))
        route.sceneConfig.asInstanceOf[NavigatorSceneConfig]
      else if (props.sceneConfig.isDefined) props.sceneConfig.get
      else if (isAndroidPlatform) NavigatorClass.SceneConfigs.FadeAndroid
      else NavigatorClass.SceneConfigs.FloatFromRight
    }

    def renderSceneF(route: js.Dynamic, navigator: NavigatorM): ReactElement = {
      if (ctrl == null) {
      ...
      MobileRouterContext(ctrl)(
        props.config.renderScene(route.asInstanceOf[NavigatorRoute], ctrl))
    }
  }

  case class Props(config: UniversalRouterConfig,
                   sceneConfig: js.UndefOr[NavigatorSceneConfig],
                   style: js.UndefOr[js.Any],
                   sceneStyle: js.UndefOr[js.Dictionary[Any]])

  def apply(routerConfig: UniversalRouterConfig,
            sceneConfig: js.UndefOr[NavigatorSceneConfig] = js.undefined,
            style: js.UndefOr[js.Any] = js.undefined,
            sceneStyle: js.UndefOr[js.Dictionary[Any]] = js.undefined) =
    makeElement[Component](Props(routerConfig, sceneConfig, style, sceneStyle))
